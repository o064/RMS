...
vector<string> CLIController::tokenize(const string &args)
{ // split the sentence to args
...
void CLIController::show_help()
{
    ...
    cout << "   train get <id>                                 - Show train details\n\n";
    ...
    cout << "   passenger get <id>                             - Show passenger details\n\n";

    // ======================== TICKET ========================
    cout << "ticket:\n";
    cout << "   ticket list                                    - Show all tickets\n";
    cout << "   ticket book <trainId> <passengerName>          - Book a ticket\n";
    cout << "   ticket cancel <ticketId>                       - Cancel a ticket\n";
    cout << "   ticket get <ticketId>                          - Show ticket details\n\n";
...
void CLIController::run()
{
    ...
    static auto printUsage = [](MainCmd cmd)
    {
        ...
        {
        case MainCmd::TRAIN:
            cout << "Usage: train <list|get|add|delete|update|seats|availability|status>\n";
            break;
        case MainCmd::PASSENGER:
            cout << "Usage: passenger <list|get|add|delete|update>\n";
            break;
        case MainCmd::TICKET:
            cout << "Usage: ticket <list|get|book|cancel>\n";
            break;
        default:
    ...
    while (true)
    {
        ...
        case MainCmd::TRAIN:
        {
            ...
            case TrainCmd::ADD:
                add_train(args);
                break;
            case TrainCmd::DELETE:
                delete_train(args);
                break;
            case TrainCmd::UPDATE:
                update_train(args);
                break;
            case TrainCmd::SEATS_ADD:
                add_seats(args);
                break;
            case TrainCmd::AVAILABILITY:
                get_train_availability(args);
                break;
            case TrainCmd::STATUS:
                get_train_status(args);
                break;

            default:
                cout << "Invalid train command.\n";
            }
            break;
        }
        ...
        case MainCmd::PASSENGER:
        {
            ...
            switch (pCmd)
            {
            case PassengerCmd::GET:
                get_passenger(args);
                break;
            case PassengerCmd::LIST:
                list_passengers();
                break;
            case PassengerCmd::ADD:
                add_passenger(args);
                break;
            case PassengerCmd::DELETE:
                delete_passenger(args);
                break;
            case PassengerCmd::UPDATE:
                update_passenger(args);
                break;
            default:
                cout << "Invalid passenger command.\n";
            }
            break;
        }
        ...
        case MainCmd::TICKET:
        {
            ...
            case TicketCmd::BOOK:
                book_ticket(args);
                break;
            case TicketCmd::CANCEL:
                cancel_ticket(args);
                break;
            default:
                cout << "Invalid ticket command.\n";
            }
            break;
        }
...
void CLIController::add_passenger(const vector<string> &args)
{
    ...
    {
        std::cerr << "\033[31m" << "ERROR: Could not add passenger " << name << "." << "\n";
        std::cerr << "Details: " << e.what() << "\033[0m" << "\n";
    }
}
...
void CLIController::get_train_availability(const vector<string> &args)
{
    ...
    {
        std::cerr << "\033[31m" << "Error while get_train_availability : " << e.what() << "\033[0m" << endl;
        return;
    }
}
...
void CLIController::add_train(const vector<string> &args)
{
    ...
    {
        std::cerr << "\033[31m" << "ERROR: Could not add train " << name << ". " << e.what() << "\033[0m" << "\n";
    }
}
void CLIController::book_ticket(const vector<string> &args)
{
    if (args.size() < 4)
    { // book ticket <train_id> <passenger_name>
    ...
    {
        std::cerr << "\033[31m" << "ERROR: Could not book ticket for " << passengerName << " on Train ID " << trainIdArg << "." << "\n";
        std::cerr << "Details: " << e.what() << "\033[0m" << "\n";
    }
}
...
void CLIController::cancel_ticket(const vector<string> &args)
{
    ...
    {
        // Error Message
        std::cerr << "\033[31m" << "ERROR: Could not cancel ticket " << ticketIdArg << "." << "\n";
        std::cerr << "Details: " << e.what() << "\033[0m" << "\n";
    }
}
...
void CLIController::update_train(const vector<string> &args)
{
    ...
    {
        std::cerr << "\033[31m" << "ERROR: Could not updateTrain " << tarinIdArg << "." << "\n";
        std::cerr << "Details: " << e.what() << "\033[0m" << "\n";
    }
}
...
void CLIController::add_seats(const vector<string> &args)
{
    ...
    {
        std::cerr << "\033[31m" << "ERROR: Could not update Train " << trainArg << "." << "\n";
        std::cerr << "Details: " << e.what() << "\033[0m" << "\n";
    }
}
...
void CLIController::update_passenger(const vector<string> &args)
{
    ...
    {
        std::cerr << "\033[31m" << "ERROR: Could not update passenger " << passengerArg << "." << "\n";
        std::cerr << "Details: " << e.what() << "\033[0m" << "\n";
    }
}
...
void CLIController::delete_train(const vector<string> &args)
{
    ...
    try
    {
        ...
        cout << "\033[32m" << "train with id " << trainId << " deleted successfully \n"
             << "\033[0m";
    }
    catch (const exception &e)
    {
        std::cerr << "\033[31m" << "ERROR: Could not delete train" << "." << "\n";
        std::cerr << "Details: " << e.what() << "\033[0m" << "\n";
    }
}
...
void CLIController::delete_passenger(const vector<string> &args)
{
    ...
    try
    {
        ...
        cout << "\033[32m" << "passenger with id " << passengerId << " deleted successfully \n"
             << "\033[0m";
    }
    catch (const exception &e)
    {
        std::cerr << "\033[31m" << "ERROR: Could not delete passenger" << "." << "\n";
        std::cerr << "Details: " << e.what() << "\033[0m" << "\n";
    }
}
...
void CLIController::get_train_status(const vector<string> &args)
{
    ...
    {
        std::cerr << "\033[31m" << "ERROR: could not print train data with id : " << trainArg << "." << "\n";
        std::cerr << "Details: " << e.what() << "\033[0m" << "\n";
    }
}

void CLIController::get_train(const vector<string> &args)
{
    if (args.size() < 3)
    {
        cout << "Usage: train get <trainId>\n";
        return;
    }
    const string &trainArg = args[2];
    try
    {
        int trainId = parseInt(trainArg, "train Id");
        auto t = facade->getTrain(trainId);
        cout << "\033[32m";
        t.print("--- Train Details ---\n");
        cout << "\033[0m";
    }
    catch (const exception &e)
    {
        std::cerr << "\033[31m" << "ERROR: Could not fetch train " << trainArg << "." << "\n";
        std::cerr << "Details: " << e.what() << "\033[0m" << "\n";
    }
}

void CLIController::get_passenger(const vector<string> &args)
{
    if (args.size() < 3)
    {
        cout << "Usage: passenger get <passengerId>\n";
        return;
    }
    const string &passengerArg = args[2];
    try
    {
        int passengerId = parseInt(passengerArg, "passenger Id");
        auto p = facade->getPassenger(passengerId);
        cout << "\033[32m";
        p.print("--- Passenger Details ---\n");
        cout << "\033[0m";
    }
    catch (const exception &e)
    {
        std::cerr << "\033[31m" << "ERROR: Could not get passenger " << passengerArg << "." << "\n";
        std::cerr << "Details: " << e.what() << "\033[0m" << "\n";
    }
}

void CLIController::get_ticket(const vector<string> &args)
{
    if (args.size() < 3)
    {
        cout << "Usage: ticket get <ticketId>\n";
        return;
    }
    const string &ticketArg = args[2];
    try
    {
        int ticketId = parseInt(ticketArg, "ticket Id");
        auto t = facade->getTicket(ticketId);
        cout << "\033[32m";
        t.print("--- Ticket Details ---\n");
        cout << "\033[0m";
    }
    catch (const exception &e)
    {
        std::cerr << "\033[31m" << "ERROR: Could not get ticket " << ticketArg << "." << "\n";
        std::cerr << "Details: " << e.what() << "\033[0m" << "\n";
    }
}
